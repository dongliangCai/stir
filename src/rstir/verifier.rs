...
impl<F: FftField> VerificationState<F> {
    // query g_i at a given point.
    // This induces some query to the previous oracle(f_i)
    pub fn query(
        &self,
        evaluation_point: F,
...
pub struct RstirVerifier<F, MerkleConfig, FSConfig>
where
    F: FftField,
    MerkleConfig: Config,
...
impl<F, MerkleConfig, FSConfig> Verifier<F, MerkleConfig, FSConfig>
    for RstirVerifier<F, MerkleConfig, FSConfig>
where
    ...
    FSConfig::Config: Clone,
{
    ...
    fn verify(
        ...
        for round_proof in &proof.round_proofs {
            ...
            current_root = round_proof.root.clone();
        }
        if !proof
            .queries_to_final
            .1
        ...
        for round_proof in &proof.round_proofs {
            let round_result = self.round(ni, &mut sponge, round_proof, verification_state);
...
impl<F, MerkleConfig, FSConfig> RstirVerifier<F, MerkleConfig, FSConfig>
where
    F: FftField + PrimeField + Absorb,
    MerkleConfig: Config<Leaf = Vec<F>>,
    MerkleConfig::InnerDigest: Absorb,
    ...
    FSConfig::Config: Clone,
{
    ...
    fn round(
        ...
        // Redo FS
        sponge.absorb(&round_proof.root);
        ...
        sponge.absorb(&round_proof.gammas);
        // let comb_randomness = sponge.squeeze_field_elements(self.parameters.ood_samples + self.parameters.repetitions[verification_state.num_round]);
        // let new_folding_randomness = sponge.squeeze_field_elements(log2(self.parameters.folding_factor) as usize);
        ...
        // First, we need to query the previous oracle (which is either h_0 or f_i)
        ...
        let mut folded_answers = Vec::new();

        if verification_state.num_round == 0 {
        } else {
